...
fn select_output_device(host: &cpal::Host, output_device: Option<&str>) -> Result<cpal::Device> {
...
fn select_output_config(
        ...
        .context("failed to query supported output configs")?
    {
        ...
        let Some(rank) = output_sample_format_rank(config.sample_format()) else {
            continue;
        };
...
/// Ranks CPAL device output sample formats for selection.
///
/// AES67 RTP payloads are decoded into internal `f32` samples. This ranking is
/// only about the native sample format requested by the selected audio output
/// device. Prefer `F32` to avoid conversion, then fall back to common PCM
/// formats that can be converted from `f32` in the CPAL callback.
fn output_sample_format_rank(output_format: SampleFormat) -> Option<u8> {
    match output_format {
...
/// Builds a CPAL output stream whose callback buffer type matches the selected
/// device output sample format.
///
/// CPAL reports the sample format at runtime, but `build_output_stream` is
/// generic over the concrete callback sample type. This dispatch bridges that
/// boundary by choosing the matching `build_stream::<T>` instantiation.
