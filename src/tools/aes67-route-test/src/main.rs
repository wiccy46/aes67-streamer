use aes67_engine::sender::{Aes67Sender, SendAudioSource, SenderConfig};
...
async fn main() {
    ...
    if let Err(error) = run(args).await {
        log::error!("AES67 route test failed: {error:#}");
        std::process::exit(1);
    }
}
...
async fn run(args: TesterArgs) -> Result<()> {
    ...
    let mut sender = Aes67Sender::new_with_audio_source(
        Box::new(source),
    ...
    let transmitter_rtp_base_timestamp = sender.get_rtp_base_timestamp();
    ...
    let transmitter =
        tokio::spawn(async move { sender.run_until_cancelled(transmitter_shutdown).await });
...
impl SendAudioSource for SineSource {
