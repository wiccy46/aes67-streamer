...
fn help_exits_successfully_and_hides_test_only_output() {
    ...
    assert!(logs.contains("Usage: aes67 receive listen"));
...
fn version_exits_successfully() {
    ...
    assert_eq!(logs.trim(), format!("aes67 {}", version_from_file()));
}
...
fn nonexistent_output_device_exits_with_clear_error() {
    ...
    assert_failure_contains(&output, "Failed to create AES67 receiver");
}
