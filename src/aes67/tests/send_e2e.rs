...
async fn test_e2e_streaming_with_cli_args() -> Result<()> {
    run_streaming_test(SenderArgs::Cli).await
}

#[tokio::test]
async fn test_e2e_streaming_with_config_file() -> Result<()> {
    run_streaming_test(SenderArgs::ConfigFile).await
}

#[cfg(unix)]
#[tokio::test]
async fn test_e2e_streaming_stops_gracefully_on_sigterm() -> Result<()> {
    run_streaming_test(SenderArgs::Sigterm).await
}

#[cfg(unix)]
#[tokio::test]
async fn test_e2e_streaming_stops_gracefully_on_sigint() -> Result<()> {
    run_streaming_test(SenderArgs::Sigint).await
}

#[tokio::test]
async fn test_e2e_loop_playback_streams_past_end_of_short_file() -> Result<()> {
    run_streaming_test(SenderArgs::LoopPlayback).await
}

#[tokio::test]
async fn test_e2e_stream_metadata_controls_rtp_header() -> Result<()> {
    run_streaming_test(SenderArgs::Metadata).await
}

#[tokio::test]
async fn test_e2e_send_file_writes_sdp_output_file() -> Result<()> {
    run_streaming_test(SenderArgs::SdpOutput).await
}
...
async fn run_streaming_test(args_source: SenderArgs) -> Result<()> {
    let short_loop_file = if matches!(args_source, SenderArgs::LoopPlayback) {
    ...
    let binary_path = option_env!("CARGO_BIN_EXE_aes67")
        .map(PathBuf::from)
        .unwrap_or_else(|| {
            PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../target/debug/aes67")
        });
    let (multicast_addr, preferred_port) = match args_source {
        SenderArgs::Cli => ("239.1.2.3", 55005),
        SenderArgs::ConfigFile => ("239.1.2.4", 55006),
        SenderArgs::Sigterm => ("239.1.2.5", 55007),
        SenderArgs::Sigint => ("239.1.2.6", 55008),
        SenderArgs::LoopPlayback => ("239.1.2.7", 55009),
        SenderArgs::Metadata => ("239.1.2.8", 55010),
        SenderArgs::SdpOutput => ("239.1.2.9", 55011),
    };
    let sdp_output = if matches!(args_source, SenderArgs::SdpOutput) {
        Some(std::env::temp_dir().join(format!(
            "aes67-send-e2e-{}-{port}.sdp",
            std::process::id(),
            port = preferred_port
        )))
    } else {
    ...
    command.kill_on_drop(true).arg("send").arg("file");

    match args_source {
        SenderArgs::Cli => {}
        SenderArgs::ConfigFile => {
        ...
        SenderArgs::Sigterm => {}
        SenderArgs::Sigint => {}
        SenderArgs::LoopPlayback => {}
        SenderArgs::SdpOutput => {}
    };
    ...
    if matches!(args_source, SenderArgs::LoopPlayback) {
        command.arg("--loop");
    }
    ...
    if !matches!(args_source, SenderArgs::Sigterm | SenderArgs::Sigint) {
        let duration = if matches!(args_source, SenderArgs::LoopPlayback) {
            "0.2"
        } else {
            "2"
        };
    ...
    loop {
        tokio::select! {
            ...
            Ok((len, _)) = listener.recv_from(&mut buf) => {
                if len > 0 {
                    ...
                    let expected_payload_type = if matches!(args_source, SenderArgs::Metadata) {
                        101
                    } else {
                        97
                    };
                    ...
                    if matches!(args_source, SenderArgs::Metadata) {
                        assert_eq!(
                            u32::from_be_bytes([buf[8], buf[9], buf[10], buf[11]]),
                            3735928559,
                            "SSRC should match stream metadata"
                        );
                    } else if matches!(args_source, SenderArgs::Cli | SenderArgs::ConfigFile) {
        ...
        let target_packets = if matches!(args_source, SenderArgs::LoopPlayback) {
            50
        } else {
            100
        };
    ...
    if matches!(args_source, SenderArgs::Sigterm | SenderArgs::Sigint) {
        ...
        let signal = match args_source {
            SenderArgs::Sigterm => "-TERM",
            SenderArgs::Sigint => "-INT",
            _ => unreachable!("non-signal sender args should not reach signal branch"),
        };
        let status = tokio::process::Command::new("kill")
            ...
            .status()
            .await?;
        assert!(status.success(), "failed to send {signal} to sender");
    }
    ...
    assert!(status.success(), "sender exited with {status}");
...
fn resource_config_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../tests/resources/send/streaming/e2e-send-file.toml")
}

fn resource_metadata_config_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../tests/resources/send/streaming/e2e-metadata.toml")
}

fn audio_format_resource(filename: &str) -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../tests/resources/send/streaming/audio-formats")
        .join(filename)
}
...
async fn run_audio_format_streaming_test(filename: &str, index: usize) -> Result<()> {
    ...
    let status = time::timeout(Duration::from_secs(5), child.wait()).await??;
    assert!(status.success(), "{filename} sender exited with {status}");
    assert!(
        packets_received > 0,
...
fn create_short_loop_wav() -> Result<PathBuf> {
    let path = std::env::temp_dir().join(format!("aes67-send-loop-e2e-{}.wav", std::process::id()));
