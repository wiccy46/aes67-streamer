...
fn version_exits_successfully_and_matches_version_file() {
    let output = Command::new(env!("CARGO_BIN_EXE_aes67"))
        .args(["receive", "discover", "--version"])
        .output()
        .expect("aes67 receive discover --version should run");
...
fn help_exits_successfully_and_hides_test_only_options() {
    let output = Command::new(env!("CARGO_BIN_EXE_aes67"))
        .args(["receive", "discover", "--help"])
        .output()
        .expect("aes67 receive discover --help should run");
