...
fn once_mode_prints_real_sap_datagram_and_writes_sdp() {
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_aes67-sap"))
        .arg("--interface")
        .arg("127.0.0.1")
        .arg("--once")
        .arg("--sdp-output-dir")
    ...
    loop {
        ...
        if child
            .try_wait()
            .expect("aes67-sap status should be readable")
            .is_some()
        {
            break;
        }
        if Instant::now() >= deadline {
            ...
            let output = child
                .wait_with_output()
                .expect("aes67-sap output should be readable after timeout");
            panic!(
                "aes67-sap did not exit after SAP datagram\n{}",
                output_text(&output)
            );
        }
    ...
    let output = child
        .wait_with_output()
        .expect("aes67-sap output should be readable");
    assert!(
        output.status.success(),
        "aes67-sap should exit successfully\n{}",
        output_text(&output)
    );
