...
fn test_audio_file_loading_integration() {
    ...
    let test_file = test_resource("piano_freesound.wav");
    ...
    if test_file.exists() {
        ...
        println!(
            "✅ Audio file loaded: {} Hz, {} channels",
            info.sample_rate, info.channels
        );
    } else {
        panic!("Test audio file not found: {}", test_file.display());
    }
}
...
fn test_common_audio_file_formats_load() {
    for filename in ["tone.wav", "tone.flac", "tone.mp3", "tone.aiff"] {
        ...
        assert_eq!(
            ...
            "{filename} should target AES67 sample rate"
        );
        assert_eq!(info.channels, 2, "{filename} should preserve stereo layout");
        assert!(
            info.duration
                .is_some_and(|duration| duration.as_millis() > 0),
...
fn test_error_handling_integration() {
    ...
    let result = AudioReader::new(test_resource("../Cargo.toml"));
    assert!(result.is_err());
...
fn test_multi_channel_processing() {
    ...
    if test_file.exists() {
        ...
        if let Some(sample) = reader.read_next_frame().unwrap() {
            ...
            println!(
                "✅ Multi-channel format verified: {} frames, {} channels, {} samples",
                sample.frames,
                sample.channels,
                sample.data.len()
            );
