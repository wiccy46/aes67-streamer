use super::source::SendAudioSource;
...
pub struct Aes67Sender {
    audio_source: Box<dyn SendAudioSource>,
...
/// An Aes67Sender is the sender entry point for an AES67 RTP session.
...
impl Aes67Sender {
    pub async fn new(
        ...
        log::info!("Initializing AES67 sender...");
    ...
    async fn build_with_audio_source(
        ...
        log::info!("AES67 sender initialized successfully");
...
fn build_sdp(context: &SdpContext, clock_identity: ClockIdentity, config: &SenderConfig) -> String {
    format!(
        "v=0\r\n\
...
mod tests {
    ...
    async fn test_sender_creation() {
        ...
        if std::path::Path::new(test_file).exists() {
            let config = SenderConfig::default();
            let sender =
                Aes67Sender::new(test_file, "239.192.1.1", 5004, Some("127.0.0.1"), config).await;
    ...
    #[tokio::test]
    async fn sender_exposes_loaded_audio_info() {
        let source = crate::sender::SilenceSource::new(2, 48_000, 48);
        let sender = Aes67Sender::new_with_audio_source(
            Box::new(source),
            "239.192.1.1",
            5004,
            Some("127.0.0.1"),
        ...
        let info = sender.get_audio_info();
    ...
    #[tokio::test]
    async fn sender_creation_rejects_more_than_eight_channels() {
        let test_file = create_test_wav_file(9, 48);

        let result = Aes67Sender::new(
    ...
    fn writes_sdp_file_and_creates_parent_directory() {
        let path = std::env::temp_dir()
            .join(format!("aes67-sender-sdp-test-{}", std::process::id()))
            .join("stream.sdp");
    ...
    fn create_test_wav_file(channels: u16, frames: usize) -> PathBuf {
        let path = std::env::temp_dir().join(format!(
            "aes67-sender-format-validation-{}-{}ch.wav",
            std::process::id(),
            channels
        ));
