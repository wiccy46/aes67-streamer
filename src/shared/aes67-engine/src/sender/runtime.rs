...
impl RuntimeSupervisor {
    ...
    pub async fn run_sender(&self, sender: &mut Aes67Sender) -> Result<()> {
        let signal_token = self.get_shutdown_token();
        let stream_token = self.get_shutdown_token();
        let signal_task = tokio::spawn(async move {
            wait_for_os_shutdown().await;
            signal_token.cancel();
        });

        let result = sender.run_until_cancelled(stream_token).await;
        signal_task.abort();
