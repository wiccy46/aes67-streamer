use super::{Aes67Sender, SenderConfig};
...
const CONFIG_DIR_ENV: &str = "AES67_SEND_QUEUE_CONFIG_DIR";
const SETTINGS_FILE: &str = "send-queue.toml";
const ACCENT_COLOR: Color = Color::Rgb(214, 132, 58);
const COMPLETION_LIST_LIMIT: usize = 10;

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
struct SendQueueSettings {
...
impl Default for SendQueueSettings {
    fn default() -> Self {
        Self {
            stream: StreamSettings {
                address: String::new(),
                port: 5004,
                interface: None,
                session_name: "AES67 Send Queue".to_string(),
                sap: true,
                ptp_domain: 0,
                payload_type: 97,
                packet_time_ms: 1,
...
impl SendQueueApp {
    fn new(settings: SendQueueSettings, settings_path: PathBuf, settings_created: bool) -> Self {
    ...
    fn handle_key(&mut self, key: KeyEvent) -> Result<()> {
        ...
        match self.screen {
            AppScreen::Queue => self.handle_queue_key(key),
...
pub async fn run() -> Result<()> {
    ...
    let app = SendQueueApp::new(settings, settings_path, settings_created);
...
async fn run_terminal_app(mut app: SendQueueApp) -> Result<()> {
...
async fn poll_active_stream(
    ...
    let result = match active.handle.await {
        Ok(result) => result,
        Err(error) => Err(anyhow!("send queue task failed: {error}")),
    };
...
fn spawn_music_stream(request: PlaybackStartRequest) -> ActiveMusicStream {
    ...
    let handle = tokio::spawn(async move {
        let config = sender_config_from_settings(&request.stream);
        ...
        .await?;
        let duration = sender.get_audio_info().duration;
        let _ = event_tx.send(PlaybackRuntimeEvent::Started {
            track_index,
            duration,
        });
        sender.run_until_cancelled(task_shutdown).await
    });
...
async fn shutdown_active_stream(active_stream: Option<ActiveMusicStream>) -> Result<()> {
    ...
    active
        .handle
        .await
        .map_err(|error| anyhow!("send queue task failed: {error}"))??;
    Ok(())
}
...
fn render_app(frame: &mut Frame<'_>, app: &SendQueueApp) {
    let area = frame.area();
    render_queue_surface(frame, app, area);
...
fn render_queue_surface(frame: &mut Frame<'_>, app: &SendQueueApp, area: Rect) {
...
fn render_playlist(frame: &mut Frame<'_>, app: &SendQueueApp, area: Rect) {
...
fn render_side_panel(frame: &mut Frame<'_>, app: &SendQueueApp, area: Rect) {
    ...
    let now_playing = Paragraph::new(vec![
        ...
        Line::from(match app.playback_state {
            ...
            PlaybackState::Starting { .. } => "Preparing AES67 sender.",
...
fn render_stream_meter(frame: &mut Frame<'_>, app: &SendQueueApp, area: Rect) {
...
fn render_settings_modal(frame: &mut Frame<'_>, app: &SendQueueApp, area: Rect) {
    let block = Block::default()
...
fn render_app_to_string(app: &SendQueueApp, width: u16, height: u16) -> Result<String> {
...
fn load_or_create_settings_with_state(path: &Path) -> Result<(SendQueueSettings, bool)> {
    if path.exists() {
    ...
    let settings = SendQueueSettings::default();
    save_settings(path, &settings)?;
    Ok((settings, true))
}

fn save_settings(path: &Path, settings: &SendQueueSettings) -> Result<()> {
...
fn sender_config_from_settings(stream: &StreamSettings) -> SenderConfig {
    SenderConfig {
...
mod tests {
    ...
    fn temp_settings_file(name: &str) -> PathBuf {
        let dir = env::temp_dir().join(format!("aes67-send-queue-{name}-{}", std::process::id()));
    ...
    fn configured_app(name: &str) -> (SendQueueApp, PathBuf) {
        let path = temp_settings_file(name);
        let mut settings = SendQueueSettings::default();
        settings.stream.address = "239.69.83.1".to_string();
        settings.stream.interface = Some("en0".to_string());
        (
            SendQueueApp::new_with_interfaces(
                settings,
                path.clone(),
                false,
    ...
    fn type_path(app: &mut SendQueueApp, path: &Path) {
    ...
    fn default_settings_do_not_include_stream_address() {
        assert_eq!(SendQueueSettings::default().stream.address, "");
    }
    ...
    fn first_run_persists_default_settings() {
        ...
        assert_eq!(settings, SendQueueSettings::default());
    ...
    fn field_edit_updates_stream_address() {
        ...
        let mut settings = SendQueueSettings::default();
        settings.stream.interface = Some("en0".to_string());
        let mut app = SendQueueApp::new(settings, path.clone(), false);
    ...
    fn missing_settings_launches_with_settings_modal_open() {
        ...
        let (settings, created) =
            load_or_create_settings_with_state(&path).expect("settings should load");
        let app = SendQueueApp::new(settings, path.clone(), created);
    ...
    #[test]
    fn existing_settings_launches_on_queue_screen() {
        ...
        assert!(!created);
        assert_eq!(app.screen, AppScreen::Queue);
        assert!(!app.settings_required);
    ...
    fn settings_key_reopens_same_settings_modal() {
        ...
        let mut app = SendQueueApp::new(SendQueueSettings::default(), path.clone(), false);
    ...
    fn interface_field_enter_opens_interface_picker() {
        ...
        let mut app = SendQueueApp::new_with_interfaces(
            SendQueueSettings::default(),
            path.clone(),
            false,
            vec![interface("en0", [192, 168, 1, 42])],
        );
    ...
    #[test]
    fn queue_selection_moves_with_queue_arrow_keys() {
    ...
    #[test]
    fn stream_settings_build_sender_config_for_queue_mode() {
        ...
        let config = sender_config_from_settings(&stream);
    ...
    #[test]
    fn ratatui_renderer_shows_queue_surface() {
        let path = temp_settings_file("render-queue");
        let mut settings = SendQueueSettings::default();
        settings.stream.address = "239.69.83.1".to_string();
        settings.stream.interface = Some("en0".to_string());
        let app = SendQueueApp::new(settings, path.clone(), false);

        let output = render_app_to_string(&app, 100, 30).expect("app should render");

        assert!(output.contains("AES67 Send Queue"));
    ...
    fn ratatui_renderer_shows_settings_modal() {
        ...
        let app = SendQueueApp::new(SendQueueSettings::default(), path.clone(), true);
